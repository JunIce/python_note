...
'''
'''
当使用松散正则表达式时，最重要的一件事情就是:必须传递一个额外的 参数 re.VERBOSE，该参数是定义在 re 模块中的一个常量，标志着待匹配的 正则表达式是一个松散正则表达式。
这个模式匹配字符串的开始，接着匹配三个可选 M 字符中的一个，接着 匹配 CM，接着是字符 L 和三个可选 X 字符的所有字符，接着是 IX，然后 是字符串的结尾。
'''